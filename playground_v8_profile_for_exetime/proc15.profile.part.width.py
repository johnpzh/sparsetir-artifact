...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-partitions", "-p", type=str, help="number of partitions")
    parser.add_argument("--max-width", "-w", type=str, help="max bucket width")
    if len(sys.argv) == 1:
    ...
    MAX_BUCKET_WIDTH = int(args.max_width)
    ...
    for name in ["reddit"]:
        ...
        if format == "CELL":
            ...
            for num_features in [512]:
                ...
                max_bucket_width = MAX_BUCKET_WIDTH
                # bucket_config = []
                # for i_p in range(num_parts):
                #     buckets = []
                #     width = 1
                #     while width <= max_bucket_width:
                #         buckets.append(width)
                #         width *= 2
                #     bucket_config.append(buckets)
                init_buckets = fa.get_init_buckets(g, 
                                                   num_parts=num_parts, 
                                                   width_limit=max_bucket_width)
                cost_sum = 0
                bucket_config = []
                for part_ind in range(len(init_buckets)):
                    bucket_dict = init_buckets[part_ind]
                    cost_dict = fa.get_memory_cost(bucket_dict=bucket_dict,
                                                   cost_model_config=cost_model_config)
                    cost_sum += cost_dict["cost"]
                    bucket_list = fa.get_bucket_widths_list(bucket_dict)
                    bucket_config.append(bucket_list)

                time_bucket_end = time.perf_counter()
                time_bucket = time_bucket_end - time_bucket_start
                print(f"name: {name} feat_size: {num_features} num_parts: {num_parts} max_bucket_width: {max_bucket_width} cost: {cost_sum} bucket_config: {bucket_config}", flush=True)
                ...
                try:
                    ...
                    print(f"name: {name} format: {format} feat_size: {num_features} num_partitions: {num_parts} max_bucket_width: {max_bucket_width}  cost: {cost_sum} bucket_config: {bucket_config} exe_time: {exe_time}", flush=True)
                ...
                max_width_list.append(max_bucket_width)
                cost_list.append(cost_sum)
                time_selet_list.append(time_select)
    ...
    table = {
        ...
        "max_bucket_width": max_width_list,
        "cost": cost_list,
    ...
    print(df.to_string())
    df.to_csv(f"output.{sys.argv[0]}.part{num_parts}.width{max_bucket_width}.csv", index=False)
