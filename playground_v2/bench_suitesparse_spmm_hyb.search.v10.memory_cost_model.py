...
def driver(init_buckets: list,
           ...
           memory_cost: dict):
    # Run
    # final_bucket_config = [format_algos.get_bucket_widths_list(bucket_dict)]
    final_bucket_config = []
    for part_ind in range(num_parts):
        final_bucket_config.append(format_algos.get_bucket_widths_list(init_buckets[part_ind]))
    ...
    memory_cost["exe_time(ms)"].append(exe_time)
    # End run
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-partitions", "-p", type=int, help="number of partitions")
    ...
    cost_model_config = CostModelSettings(feat_size=feat_size,
                                          num_parts=num_parts)
    # test
    print(F"\nold buckets...")
    for part_ind in range(num_parts):
    ...
    for part_ind in range(num_parts):
        bucket_dict = init_buckets[part_ind]
        memory_cost = {
            "max_bucket_width": [],
            "exe_time(ms)": [],
            "memory_cost": [],
        ...
        old_max_bucket_width = max(bucket_dict.keys())
        if old_max_bucket_width != new_max_width:
            old_w = old_max_bucket_width
            memory_cost["max_bucket_width"].append(old_w)
            # cost, cost1, cost2, cost3 = format_algos.get_memory_cost(bucket_dict=bucket_dict, 
            #                                                 feat_size=feat_size, 
            #                                                 num_parts=num_parts)
            cost_dict = format_algos.get_memory_cost(bucket_dict=bucket_dict, 
                                                     cost_model_config=cost_model_config)
                                                    # feat_size=feat_size, 
                                                    # num_parts=num_parts)
            cost = cost_dict["cost"]
            cost1 = cost_dict["cost1"]
            cost2 = cost_dict["cost2"]
            cost3 = cost_dict["cost3"]
            load_imbalance_penalty = cost_dict["load_imbalance_penalty"]
            memory_cost["memory_cost"].append(cost)
            memory_cost["2IK"].append(cost1)
            memory_cost["|set(col_ind[i,k])|J"].append(cost2)
            memory_cost["IJ"].append(cost3)
            memory_cost["penalty"].append(load_imbalance_penalty)
            # test
            # print("")
            # print(F"init_buckets[{part_ind}]:")
            # rows_in_bucket = 0
            # the_max = bucket_dict.keys()
            # for width, bucket in  bucket_dict.items():
            #     print(F"{width}: {bucket}")
            #     num_rows = bucket.num_rows
            print("")
            format_algos.print_a_bucket(bucket_dict=bucket_dict,
                                        part_ind=part_ind)
            # end test

            # driver(init_buckets=init_buckets,
            #        g=g,
            #        name=name,
            #        feat_size=feat_size,
            #        num_parts=num_parts,
            ...
            memory_cost["exe_time(ms)"] = 0
                
            best_cost = cost
            best_max_bucket_width = old_max_bucket_width
            while old_w != new_max_width:
                new_w = old_w // 2
                # memory_overhead += format_algos.get_memory_overhead(init_buckets[0], old_w, new_w)
                format_algos.move_largest_bucket(bucket_dict, old_w, new_w)
                old_w = new_w
                memory_cost["max_bucket_width"].append(old_w)
                # cost, cost1, cost2, cost3 = format_algos.get_memory_cost(bucket_dict=bucket_dict, 
                #                                             feat_size=feat_size, 
                ...
                memory_cost["exe_time(ms)"] = 0
        # test
        print_table(memory_cost)
        print(F"name: {name}")
        print(F"best_max_bucket_width: {best_max_bucket_width}")
        pass
        # sys.exit(-1)
        # end test
