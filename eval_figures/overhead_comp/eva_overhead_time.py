...
if __name__ == "__main__":
    ...
    rects1 = axs.bar(bars1, SparseTIR_t, width=width, label="SparseTIR", color=colors[0])
    rects2 = axs.bar(bars2, STile_t, width=width, label="STile", color=colors[1])
    rects3 = axs.bar(bars3, LiteForm_t, width=width, label="LiteForm", color=colors[2])
    ...
    axs.set_xticks(bars, matrices, fontsize=29, rotation=0, ha="center")
    # axs.set_xticks(bars, matrices, rotation=45, ha="right")
    # axs.legend(loc='best', fontsize=20, ncol=1)
    axs.legend(loc='upper left', fontsize=40, ncol=1)
    ...
    # Bar label
    fontsize = 25
    axs.bar_label(rects1, fmt="%0.2f", padding=3, color=colors[0], fontsize=fontsize, rotation=90)
    axs.bar_label(rects2, fmt="%0.2f", padding=3, color=colors[1], fontsize=fontsize, rotation=90)
    axs.bar_label(rects3, fmt="%0.2f", padding=3, color=colors[2], fontsize=fontsize, rotation=90)
    ...
    fig_name_png=F"{os.path.splitext(input_file)[0]}.overhead_collect.png"
    plt.savefig(fig_name_png, dpi=300, bbox_inches="tight")

    fig_name_pdf=F"{os.path.splitext(input_file)[0]}.overhead_collect.pdf"
