...
if __name__ == "__main__":
    ...
    # axs.plot(train_data_size, similarity, color=colors[0])
    axs.scatter(train_data_size, similarity, color=colors[0], label="", edgecolors="none", alpha=0.4)
    ...
    axs.set_ylabel("Cosine similarity for number of partitions", fontsize=23)
    ...
    axs.set_xlabel("Training data size (rows)", fontsize=26)
    ...
    fig_name_png=F"{os.path.splitext(input_file)[0]}.similarity_partitions.png"
    plt.savefig(fig_name_png, dpi=300, bbox_inches="tight")

    fig_name_pdf=F"{os.path.splitext(input_file)[0]}.similarity_partitions.pdf"
