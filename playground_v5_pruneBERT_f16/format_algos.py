...
def ell(
    ...
    A = T.match_sparse_buffer(a, (O, I, J), "float16")
    T.evaluate(0)
...
def bench_hyb_with_config(g,
    ...
    for part_id in range(num_col_parts):
        ...
        for bucket_id, _ in enumerate(b_widths):
            # weight = tvm.nd.array(
            #     mask[part_id][bucket_id].numpy().reshape(-1).astype("float16"), device=tvm.cuda(0)
            # )
            # rows = tvm.nd.array(
            ...
            # )
            weight = tvm.nd.array(
                mask[part_id][bucket_id].reshape(-1).astype("float16"), device=tvm.cuda(0)
            )
            rows = tvm.nd.array(
...
def bench_tensorcores_with_config(g,
                          x,
                          # y_golden,
                          y_ndarray,
                          feat_size=128,
    ...
    for part_id in range(num_col_parts):
        ...
        for bucket_id, bucket_size in enumerate(b_widths):
            ...
            #     sch.compute_at(A_local, i)
            #     B_shared = sch.cache_read(blk, 2, "shared")
            #     sch.compute_at(B_shared, foi)
            #     B_local = sch.cache_read(blk, 2, "wmma.matrix_b")
            #     sch.compute_at(B_local, foo)
                
            #     sch.reverse_compute_at(C_local, fi, True)
            sch.bind(fi, "threadIdx.x")
            sch.bind(foo, "blockIdx.y")
            sch.unroll(foi)
            if use_implicit_unroll:
            ...
            # print(F"part_id: {part_id} bucket_id: {bucket_id} bucket_size: {bucket_size} b_widths: {b_widths}")
            # # end test
            io, ioi, ii = sch.split(i, [None, b_widths[-1] // bucket_size, 16])
            sch.bind(io, "blockIdx.x")
            sch.bind(ii, "threadIdx.y")
            ...
            init_blk = sch.decompose_reduction(blk, fi)
            A_local = sch.cache_read(blk, 1, "wmma.matrix_a")
            sch.compute_at(A_local, ii)
            B_shared = sch.cache_read(blk, 2, "shared")
            sch.compute_at(B_shared, foo)
            B_local = sch.cache_read(blk, 2, "wmma.matrix_b")
            sch.compute_at(B_local, foi)
            sch.hide_buffer_access(blk, "read", [3])
