...
def split(feature_file: str,
    ...
    df.drop(columns=["avg_nnz_per_row",
                     "min_nnz_per_row",
                     "max_nnz_per_row",
                     "std_dev_nnz_per_row",
    ...
    train_file = os.path.join(output_dir, F"{basename}.for_partitions.train_set.csv")
    test_file = os.path.join(output_dir, F"{basename}.for_partitions.infer_set.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("split data to training set and test set for partitions")
