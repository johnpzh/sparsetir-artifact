...
def create_longformer(h, mb, window, fmt="mask", block_size=1):
    ...
    if fmt == "mask":
        mask = th.zeros(h, mb, mb, dtype=th.int32)
        mask[:, rows, cols] = 1
        return mask
    elif fmt == "bsr":
        rows = rows.numpy()
        cols = cols.numpy()
        coo = sp.coo_matrix((np.ones_like(rows), (rows, cols)), shape=(mb, mb))
        # csr = coo.tocsr()
        # return csr
        return coo
    elif fmt == "csr":
        rows = (
            ...
            + th.arange(block_size, dtype=th.int32).view(1, block_size, 1)
            + th.zeros(1, 1, block_size, dtype=th.int32)
        )
        cols = (
            cols.view(-1, 1, 1) * block_size
            + th.zeros(1, block_size, 1, dtype=th.int32)
            + th.arange(block_size, dtype=th.int32).view(1, 1, block_size)
        )
...
if __name__ == "__main__":
    ...
    g = MTX(None)
    g.name = name
    g.coo_mtx = create_longformer(1, mb, 256 // block_size, fmt="bsr")

    num_features = feat_size
    ...
    # if True:
        ...
        try:
            exe_time = num_heads * bench_hyb_with_config(g,
                                    x,
                                    # y_golden,
                                    y_ndarray,
        ...
        try:
            ...
            if name not in ['proteins', 'reddit']:
                ...
                exe_time = num_heads * bench_bsrmm(bsr_weight, x, block_size=bsize)
                print(f"name: {name} exe_time: {exe_time}")
            ...
            try:
                ...
                y_ndarray = g.dot(x.numpy())
                exe_time = num_heads * bench_naive(g,
                                    x,
                                    y_ndarray,
                                    feat_size=num_features,
                                    coarsening_factor=2,
