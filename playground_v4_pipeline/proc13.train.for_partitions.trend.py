...
def train_and_infer(train_file: str,
    ...
    # while size < len(X_train):
        ...
        similarity = cosine_similarity([y_truth], [y_predict])[0][0]
        # table["data_size"].append(percent)
        table["data_size"].append(size)
        table["predict_accuracy"].append(stat["accuracy"][0])
        table["similarity"].append(similarity)
        print(f"percent: {percent} size: {size} accuracy: {stat['accuracy'][0]} similarity: {similarity}")
